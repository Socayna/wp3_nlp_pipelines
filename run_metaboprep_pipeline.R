...
if(isexcel > 0){
  ...
  if(platform == "Nightingale"){
    ...
    if( !is.na(pfile[3,2]) ){
        ...
        mydata = read.in.nightingale( file2process = METABO_file2process, data_dir="", projectname = project )
  ...
  if(platform == "Metabolon"){
    ...
    if( !is.na(pfile[3,2]) ){
      ## Read in the raw data, excel files, write to flat text in the data directory and return all data as a list
      mydata = read.in.metabolon( file2process = METABO_file2process, data_dir="", projectname = project )
...
#########################
##
## (XII) Create ZIP file of generated data
##
#########################
...
cat(paste0("\t- generated data written to the zip file ", zip_file_path, ".\n"))
